...
import type { AdminBlogTag } from '../types/admin.types'
import * as z from 'zod'

const props = defineProps<{
  tag?: AdminBlogTag | null
  isLoading?: boolean
}>()
...
const schema = z.object({
  name: z.string().trim().min(1, 'Tag name is required').max(NAME_MAX_LENGTH),
...
watch([isOpen, () => props.tag], ([open, tag]) => {
  if (open) {
    state.name = tag?.name ?? ''
    state.slug = tag?.slug ?? ''
    slugManuallyEdited.value = !!tag
  }
}, { immediate: true })
      ...
      <UCard>
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-sm text-muted">
                {{ isEditing ? 'Update the tag details' : 'Add a new blog tag' }}
              </p>
            </div>
          </div>
        </template>
        ...
        >
          ...
          >
            <UInput
              v-model="state.name"
              placeholder="e.g. JavaScript, DevOps"
